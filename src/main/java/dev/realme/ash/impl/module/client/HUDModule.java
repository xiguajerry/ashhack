...
extends ToggleModule {
    final Config<Boolean> watermarkConfig = new BooleanConfig("Watermark", "Displays client name and version watermark", true);
    final Config<Boolean> directionConfig = new BooleanConfig("Direction", "Displays facing direction", true);
    final Config<Boolean> totemConfig = new BooleanConfig("Totem", "", false);
    final Config<Boolean> armorConfig = new BooleanConfig("Armor", "Displays player equipped armor and durability", true);
    final Config<VanillaHud> potionHudConfig = new EnumConfig<>("PotionHud", "Renders the Minecraft potion Hud", VanillaHud.HIDE, VanillaHud.values());
    final Config<VanillaHud> itemNameConfig = new EnumConfig<>("ItemName", "Renders the Minecraft item name display", VanillaHud.HIDE, VanillaHud.values());
    final Config<Boolean> potionEffectsConfig = new BooleanConfig("PotionEffects", "Displays active potion effects", true);
    final Config<Boolean> potionColorsConfig = new BooleanConfig("PotionColors", "Displays active potion colors", true);
    final Config<Boolean> durabilityConfig = new BooleanConfig("Durability", "Displays the current held items durability", false);
    final Config<Boolean> coordsConfig = new BooleanConfig("Coords", "Displays world coordinates", true);
    final Config<Boolean> netherCoordsConfig = new BooleanConfig("NetherCoords", "Displays nether coordinates", true, () -> this.coordsConfig.getValue());
    final Config<Boolean> serverBrandConfig = new BooleanConfig("ServerBrand", "Displays the current server brand", false);
    final Config<Boolean> speedConfig = new BooleanConfig("Speed", "Displays the current movement speed of the player in kmh", true);
    final Config<Boolean> pingConfig = new BooleanConfig("Ping", "Display server response time in ms", true);
    final Config<Boolean> tpsConfig = new BooleanConfig("TPS", "Displays server ticks per second", true);
    final Config<Boolean> fpsConfig = new BooleanConfig("FPS", "Displays game FPS", true);
    final Config<Boolean> arraylistConfig = new BooleanConfig("Arraylist", "Displays a list of all active modules", true);
    final Config<Boolean> turtle = new BooleanConfig("Turtle", "", true);
    final Config<Boolean> all = new BooleanConfig("All", "", true);
    final Config<Float> xOffset1 = new NumberConfig<>("XOffset1", "", -500.0f, 0.0f, 500.0f);
    final Config<Float> yOffset1 = new NumberConfig<>("YOffset1", "", -500.0f, 0.0f, 500.0f);
    final Config<Float> xOffset2 = new NumberConfig<>("XOffset2", "", -500.0f, 7.0f, 500.0f);
    final Config<Float> yOffset2 = new NumberConfig<>("YOffset2", "", -500.0f, 17.0f, 500.0f);
    final Config<Ordering> orderingConfig = new EnumConfig<>("Ordering", "The ordering of the arraylist", Ordering.LENGTH, Ordering.values(), () -> this.arraylistConfig.getValue());
    final Config<Rendering> renderingConfig = new EnumConfig<>("Rendering", "The rendering mode of the HUD", Rendering.UP, Rendering.values());
    final Config<RainbowMode> rainbowModeConfig = new EnumConfig<>("Rainbow", "The rendering mode for rainbow", RainbowMode.OFF, RainbowMode.values());
    final Config<Float> alphaSpeedConfig = new NumberConfig<>("Alpha-Speed", "", 0.1f, 50.0f, 1500.0f);
    final Config<Float> rainbowSpeedConfig = new NumberConfig<>("Rainbow-Speed", "The speed for the rainbow color cycling", 0.1f, 50.0f, 100.0f);
    final Config<Integer> rainbowSaturationConfig = new NumberConfig<>("Rainbow-Saturation", "The saturation of rainbow colors", 0, 35, 100);
    final Config<Integer> rainbowBrightnessConfig = new NumberConfig<>("Rainbow-Brightness", "The brightness of rainbow colors", 0, 100, 100);
    final Config<Float> rainbowDifferenceConfig = new NumberConfig<>("Rainbow-Difference", "The difference offset for rainbow colors", 0.1f, 40.0f, 100.0f);
        ...
        if (HUDModule.mc.player != null && HUDModule.mc.world != null) {
            ...
            if (this.turtle.getValue()) {
                ...
                if (count > 0) {
                    ...
                    RenderManager.renderText(event.getContext(), String.valueOf(count), (float)res.getScaledWidth() / 2.0f - (float)width2 / 2.0f + this.xOffset1.getValue(), (float)res.getScaledHeight() / 2.0f + this.yOffset1.getValue(), this.getHudColor(this.rainbowOffset));
                }
                for (StatusEffectInstance e : HUDModule.mc.player.getStatusEffects()) {
                    ...
                    RenderManager.renderText(event.getContext(), (String)string, (float)res.getScaledWidth() / 2.0f - (float)width / 2.0f + this.xOffset2.getValue(), (float)res.getScaledHeight() / 2.0f + this.yOffset2.getValue(), this.getHudColor(this.rainbowOffset));
                }
            }
            if (this.watermarkConfig.getValue()) {
            ...
            if (this.arraylistConfig.getValue()) {
            ...
            if (this.potionEffectsConfig.getValue()) {
                for (StatusEffectInstance e : HUDModule.mc.player.getStatusEffects()) {
            ...
            if (this.serverBrandConfig.getValue() && mc.getServer() != null) {
            ...
            if (this.durabilityConfig.getValue() && HUDModule.mc.player.getMainHandStack().isDamageable()) {
            ...
            if (this.pingConfig.getValue() && !mc.isInSingleplayer()) {
            ...
            if (this.directionConfig.getValue()) {
    ...
    private int rainbow(long offset) {
        float hue = (float)(((double)System.currentTimeMillis() * (double)(this.rainbowSpeedConfig.getValue() / 10.0f) + (double)(offset * 500L)) % (double)(30000.0f / (this.rainbowDifferenceConfig.getValue() / 100.0f)) / (double)(30000.0f / (this.rainbowDifferenceConfig.getValue() / 20.0f)));
        return Color.HSBtoRGB(hue, (float) this.rainbowSaturationConfig.getValue() / 100.0f, (float) this.rainbowBrightnessConfig.getValue() / 100.0f);
    }
    ...
    public int alpha(long offset) {
        ...
        float brightness = Math.abs(((float)(System.currentTimeMillis() % 2000L) / this.alphaSpeedConfig.getValue() + 50.0f / (float)offset * 2.0f) % 2.0f - 1.0f);
