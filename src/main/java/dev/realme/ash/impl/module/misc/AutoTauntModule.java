...
extends ToggleModule {
    final Config<Mode> mode = new EnumConfig<>("Mode", "", Mode.Custom, Mode.values());
    final Config<Float> range = new NumberConfig<>("Range", "", 0.0f, 13.0f, 100.0f);
    final StringConfig text = new StringConfig("Text", "", "bye");
    ...
    public void onDeath(EntityDeathEvent event) {
        ...
        if (player != AutoTauntModule.mc.player && !Managers.SOCIAL.isFriend(player.getName())) {
            if (this.range.getValue() > 0.0f) {
                assert AutoTauntModule.mc.player != null;
                if (AutoTauntModule.mc.player.distanceTo(player) > this.range.getValue()) {
                    return;
                }
            }
