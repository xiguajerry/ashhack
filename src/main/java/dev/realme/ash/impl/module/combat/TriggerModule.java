...
extends ToggleModule {
    final Config<TriggerMode> modeConfig = new EnumConfig<>("Mode", "The mode for activating the trigger bot", TriggerMode.MOUSE_BUTTON, TriggerMode.values());
    final Config<Float> attackSpeedConfig = new NumberConfig<>("AttackSpeed", "The speed to attack entities", 0.1f, 8.0f, 20.0f);
    final Config<Float> randomSpeedConfig = new NumberConfig<>("RandomSpeed", "The speed randomizer for attacks", 0.1f, 2.0f, 10.0f);
    ...
    public void onTick(TickEvent event) {
        ...
        double d = Math.random() * (double) this.randomSpeedConfig.getValue() * 2.0 - (double) this.randomSpeedConfig.getValue();
        if (buttonDown && this.triggerTimer.passed(1000.0 - Math.max((double) this.attackSpeedConfig.getValue() + d, 0.5) * 50.0)) {
